...
/*
 * Reasons (purposes) for the MR cache entry lookup:
 *   GET_REG - search before registering an MR.
 *   GET_RDMA - search before doing RDMA.
 *   PUT - search to deregister an MR.
 *   EVICT - search to evict an MR cache entry.
 *   INSERT - search before inserting an MR cache entry.
 */
enum rv_mrc_search_reason {
	RV_MRC_SEARCH_GET_REG = 0,
	RV_MRC_SEARCH_GET_RDMA,
	RV_MRC_SEARCH_PUT,
	RV_MRC_SEARCH_EVICT,
	RV_MRC_SEARCH_INSERT
};
...
static struct rv_mr_cache_entry *rv_mr_cache_search(struct rv_mr_cache *cache,
						    ...
						    enum rv_mrc_search_reason reason,
						    struct rv_mr_cached *mrc_in);
				    ...
				    const struct mmu_notifier_range *range)
{
	...
	for (mrce = rv_int_rb_iter_first(root, range->start, range->end - 1);
		...
		/*
		 * This callback could be invoked not only when the buffer is
		 * freed, but also when part of the pages need to be swapped
		 * out. At this moment, both the virtual address and the
		 * physical pages are still available. If IOs are still in
		 * progress (refcount > 0), they will be kept. The callback
		 * will be invoked again in the future when there is no IO
		 * (refcount == 0)(proven by osu_bw test). Consequently, we
		 * have no risk of leaking stale cache entries here.
		 */
		if (!mrce->refcount) {
...
again:
	...
	existing = rv_mr_cache_search(cache, mrce->addr, mrce->len,
				      mrce->access, RV_MRC_SEARCH_INSERT, NULL);
	if (existing) {
		ret = -EINVAL;
		if (existing->freeing)
			ret = -EBUSY;
	...
	if (!cache->ops->filter) {
		...
		for (mrce = rv_int_rb_iter_first(&cache->root, addr,
			...
			case RV_MRC_SEARCH_GET_REG:
				...
				break;
			case RV_MRC_SEARCH_PUT:
				mrc = container_of(mrce, struct rv_mr_cached, entry);
				if (mrce->user_refcount < 1 || mrc != mrc_in)
					continue;
				break;
			case RV_MRC_SEARCH_EVICT:
				if (mrce->refcount > 0)
					continue;
				break;
			default:
						 ...
						 bool update_hit, bool for_user)
{
	...
	spin_lock_irqsave(&cache->lock, flags);
	mrce =  rv_mr_cache_search(cache, addr, len, acc, (update_hit || for_user) ?
				   RV_MRC_SEARCH_GET_REG : RV_MRC_SEARCH_GET_RDMA, NULL);
	if (mrce) {
...
/*
 * The caller must hold the cache->lock and the lock will be
 * unlocked before the function returns.
 */
static void rv_mr_cache_remove_mrce_and_unlock(struct rv_mr_cache *cache,
					       struct rv_mr_cache_entry *mrce,
					       unsigned long flags,
					       int is_invalidate)
{
						 ...
						 struct rv_mr_cached *mrc)
{
	...
	spin_lock_irqsave(&cache->lock, flags);
	mrce = rv_mr_cache_search(cache, addr, len, acc, RV_MRC_SEARCH_PUT, mrc);
	if (mrce) {
		if (!rv_mr_cache_mrce_put(cache, mrce, true)) {
			if (mrce->freeing) {
				/*
				 * The called function will unlock
				 * cache->lock
				 */
				rv_mr_cache_remove_mrce_and_unlock(cache, mrce,
								   flags, 0);
				return NULL;
			}
 ...
 * However, if the mrce is already marked as freeing, it will be removed
 * from the cache and freed.
 */
		     ...
		     bool for_user)
{
	...
	if (!rv_mr_cache_mrce_put(cache, mrce, for_user)) {
		if (mrce->freeing) {
			/* The called function will unlock cache->lock */
			rv_mr_cache_remove_mrce_and_unlock(cache, mrce, flags,
							   0);
			return;
		}
		list_add(&mrce->list, &cache->lru_list);
			    ...
			    u64 addr, u64 len, u32 acc)
{
	...
	spin_lock_irqsave(&cache->lock, flags);
	mrce =  rv_mr_cache_search(cache, addr, len, acc, RV_MRC_SEARCH_EVICT, NULL);
	if (mrce) {
		trace_rv_mr_cache_evict_evict(mrce->addr, mrce->len,
					      mrce->access);
		/* The called function will unlock cache->lock */
		rv_mr_cache_remove_mrce_and_unlock(cache, mrce, flags, 0);
		return 0;
	}
	spin_unlock_irqrestore(&cache->lock, flags);
	return ret;
			   ...
			   int is_invalidate)
{
	...
	spin_lock_irqsave(&cache->lock, flags);
	/*
	 * Mark mrce as in the process of being freed to prevent future cache
	 * hits.
	 */
	mrce->freeing = 1;
	...
	/* The called function will unlock cache->lock */
	rv_mr_cache_remove_mrce_and_unlock(cache, mrce, flags, is_invalidate);
	return 0;
}
