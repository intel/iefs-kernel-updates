				 ...
				 bool for_user)
{
	...
	if (refcount == 1) {
		cache->stats.inuse++;
		cache->stats.inuse_bytes += mrce->len;
#ifdef NVIDIA_GPU_DIRECT
		cache->stats.inuse_a[mrce->type]++;
		cache->stats.inuse_bytes_a[mrce->type] += mrce->len;
#endif
	}
	if (for_user)
		mrce->user_refcount++;
				  ...
				  struct evict_data *evict_data, bool *stop)
{
	...
	evict_data->cleared_bytes += mrce->len;
...
again:
	...
	new_len = cache->total_size + mrce->len;
				       ...
				       struct rv_mr_cache_entry *mrce)
{
	...
	cache->stats.inuse++;
	cache->stats.inuse_bytes += mrce->len;
	cache->total_size += mrce->len;
	cache->stats.count++;
#ifdef NVIDIA_GPU_DIRECT
	cache->stats.miss_a[mrce->type]++;
	cache->stats.inuse_a[mrce->type]++;
	cache->stats.inuse_bytes_a[mrce->type] += mrce->len;
	cache->stats.total_size_a[mrce->type] += mrce->len;
			       ...
			       struct rv_mr_cache_entry *mrce)
{
	...
	cache->stats.total_size_a[RV_MRCE_TYPE_BOTH] += mrce->len;
	cache->stats.count_a[RV_MRCE_TYPE_BOTH]++;
	cache->stats.inuse_a[RV_MRCE_TYPE_BOTH]++;
	cache->stats.inuse_bytes_a[RV_MRCE_TYPE_BOTH] += mrce->len;
