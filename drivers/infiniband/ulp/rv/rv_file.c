...
int doit_gpu_pin_mmap(struct file *fp, struct rv_user *rv, unsigned long arg)
{
	...
	if (rv->state == RV_USER_ATTACHED && rv->umrs)
		...
		goto fail;
	mutex_unlock(&rv->mutex);
	ret = rv_ioctl_gpu_buf_pin_mmap(fp, &rv->umrs->gdrdata, arg);
	rv_user_mrs_put_preemptible(rv->umrs);
...
#ifdef INTEL_GPU_DIRECT
/*
 *	RV_IOCTL_GPU_MUNMAP_UNPIN, a request to munmap and unpin a gpu buffer
 *	so that it can not be accessed by CPU anymore.
 */
int doit_gpu_munmap_unpin(struct file *fp, struct rv_user *rv,
			  unsigned long arg)
{
	...
	mutex_lock(&rv->mutex);
	umrs = rv->umrs;
	if (rv->state != RV_USER_ATTACHED ||  !umrs ||
	    umrs->tgid != task_tgid(current) || !current->mm)
		goto fail;
	ret = rv_ioctl_gpu_munmap_unpin(&umrs->gdrdata, &mparams);
	trace_rv_mr_gpu_munmap_unpin(rv->rdma_mode, mparams.gpu_buf_addr,
...
static long rv_file_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	case RV_IOCTL_GPU_PIN_MMAP:
		return doit_gpu_pin_mmap(fp, rv, arg);

#endif /* NVIDIA_GPU_DIRECT || INTEL_GPU_DIRECT */
#ifdef INTEL_GPU_DIRECT
	case RV_IOCTL_GPU_MUNMAP_UNPIN:
		return doit_gpu_munmap_unpin(fp, rv, arg);
