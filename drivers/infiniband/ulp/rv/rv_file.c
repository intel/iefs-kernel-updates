...
static int doit_attach(struct rv_user *rv, unsigned long arg, u32 rev)
{
	...
	if (rev > RV_ABI_VERSION(1, 5)) {
		if (copy_from_user(&params.in, (void __user *)arg,
				   sizeof(params.in)))
			return -EFAULT;
		page_list_len = params.in.fr_page_list_len;
	} else {
		if (copy_from_user(&params.in, (void __user *)arg,
				   sizeof(struct rv_attach_params_r5_in)))
			return -EFAULT;
		params.in.fr_page_list_len = 0;
	}
	...
	trace_rv_attach_req(rv->inx, &params);
#if defined(NVIDIA_GPU_DIRECT) || defined(INTEL_GPU_DIRECT)
	...
	ret = rv_user_mrs_attach(rv, page_list_len);
	if (ret)
		goto bail_detach;
#else
	rv_user_mrs_attach(rv, page_list_len);
#endif
#if defined(NVIDIA_GPU_DIRECT) || defined(INTEL_GPU_DIRECT)
	if (gpu) {
 ...
 */
int doit_gpu_pin_mmap(struct file *fp, struct rv_user *rv, unsigned long arg, u32 rev)
{
	int ret;
