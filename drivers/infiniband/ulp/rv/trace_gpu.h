...
DEFINE_EVENT(/* event */
	rv_gdr_msg_template, rv_gdr_msg_pin_gpu_buf,
	TP_PROTO(int inx, const char *msg, u64 d1, u64 d2),
	TP_ARGS(inx, msg, d1, d2)
);
...
DECLARE_EVENT_CLASS(/* gmr */
	rv_gdr_mr_template,
	TP_PROTO(int inx, u64 addr, u64 len, u64 id, u64 base, u32 acc,
		 u32 ipc),
	TP_ARGS(inx, addr, len, id, base, acc, ipc),
	TP_STRUCT__entry(/* entry */
		__field(int, inx)
		__field(u64, addr)
		__field(u64, len)
	...
	TP_printk(/* print */
		"inx %d addr 0x%llx len %llu id %llu base 0x%llx acc 0x%x ipc 0x%x",
		__entry->inx,
		__entry->addr,
		__entry->len,
		__entry->id,
...
DEFINE_EVENT(/* event */
	rv_gdr_mr_template, rv_gdr_mr_invalidate,
	TP_PROTO(int inx, u64 addr, u64 len, u64 id, u64 base, u32 acc,
		 u32 ipc),
	TP_ARGS(inx, addr, len, id, base, acc, ipc)
);
