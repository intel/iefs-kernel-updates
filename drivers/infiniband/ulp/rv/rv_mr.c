...
static void rv_fr_inv_rkey_done_unsignaled(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	trace_rv_wc_fr_inv_rkey_done_unsignaled((u64)pool->umrs, wc->status,
...
static void rv_fr_inv_rkey_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	trace_rv_wc_fr_inv_rkey_done((u64)pool->umrs, wc->status, wc->opcode,
				     wc->byte_len, wc->ex.imm_data);
	/*
...
#endif
{
	...
	if (mparams.in.access & IBV_ACCESS_IS_GPU_ADDR) {
		...
		if (rev <= RV_ABI_VER_MINOR_1) {
			/* lacks alloc_id, which is needed for I-GPU caching */
			ret = -EINVAL;
			goto bail_unlock;
		}
#endif
