...
int rv_inv_gdr_rkey(struct rv_user_mrs *umrs, u32 access, u32 rkey)
{
	...
	if ((access & IBV_ACCESS_KERNEL) && umrs->qp)
		ret = rv_inv_rkey(umrs, umrs->qp, rkey);
	else if (!(access & IBV_ACCESS_KERNEL) && umrs->user_qp)
		ret = rv_inv_rkey(umrs, umrs->user_qp, rkey);
	else
		ret = -EINVAL;
	if (ret) {
			      ...
			      struct ib_mr *dummy_mr)
{
	...
	if (dummy_mr) {
		 ...
		 * and reap the WQEs.
		 */
		ret = rv_inv_rkey(umrs, qp, dummy_mr->rkey);
		if (unlikely(ret)) {
			rv_err(umrs->rv_inx,
			   ...
			   struct mr_info *mr)
{
	...
	mr->ib_mr = ib_alloc_mr(mr->ib_pd, IB_MR_TYPE_MEM_REG, mr->umem->nmap);
	if (IS_ERR(mr->ib_mr)) {
