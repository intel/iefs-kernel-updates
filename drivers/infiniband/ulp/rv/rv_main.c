...
struct rv_device *rv_device_get_add_user(char *dev_name, struct rv_user *rv)
{
	...
	list_for_each_entry(dev, &rv_dev_list, dev_entry) {
		if (strcmp(dev->ib_dev->name, dev_name) == 0) {
			...
			/*
			 * Ideally, we should get the limit in rv_add_one().
			 * However, tests show that calling
			 * ib_dev->ops.query_device at that time will result
			 * in a crash when the RV is loaded. So we delay it
			 * until here. Do the query outside of the spinlock
			 * because the query may sleep.
			 */
			if (!dev->max_fast_reg_page_list_len) {
				...
				ret = dev->ib_dev->ops.query_device(
					dev->ib_dev, &attr, &udata);
				spin_lock_irqsave(&rv_dev_list_lock, flags);
				if (!ret) {
