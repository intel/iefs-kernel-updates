/* SPDX-License-Identifier: (GPL-2.0 OR BSD-3-Clause) */
/*
 * Copyright(c) 2025 Intel Corporation.
 */
#if !defined(SLES15SP7_COMPAT_H)
#define SLES15SP7_COMPAT_H

#define HAVE_FALLTHROUGH
#define HAVE_ENUM_IB_UVERBS_ADVISE_MR_ADVICE
#define HAVE_IB_DEVICE_OPS
#define DEREG_MR_HAS_UDATA
#define HAVE_MMU_NOTIFIER_RANGE
#define UVERBS_DEVICE_NO_EVENTS_FILE_LIST
#define HAVE_NEW_UVERBS_ASYNC_EVENT_FILE
#define UVERBS_FILE_NO_EVENT_HANDLER
#define UVERBS_FILE_HAS_DEFAULT_ASYNC_FILE
#define UVERBS_FILE_HAVE_XARRAY_IDR
#define UVERBS_FILE_NO_IDR_LOCK
#define UVERBS_API_OBJECT_HAVE_ID
#define RDMA_LOOKUP_GET_UOBJECT_HAVE_ATTR
#define TYPE_CLASS_NO_NEEDS_KFREE_RCU
#define IB_UMEM_GET_WITH_DEVICE
#define IB_CLIENT_ADD_RETURN_INT
#define MMU_NOTIFIER_RANGE_START_USES_MMU_NOTIFIER_RANGE
#define IB_UMEM_HAS_NO_NMAP
#define IB_CM_LISTEN_WITHOUT_MASK
#define HAVE_CLASS_MODULE_CLASS_CREATE
#define NEED_CLASS_DEVNODE_CONST_DEVICE
#define MODULE_IMPORT_NS_DMA_BUF_FOR_INTEL_GPU_DIRECT
#define HAVE_PROTOTYPE_RDMA_LOOKUP_GET_UOBJECTS
#define HAVE_STRUCT_IB_UVERBS_FILE
#define HAVE_FUNC_UVERBS_TRY_LOCK_OBJECT

#ifndef HAVE_MOFED
#define HAVE_ATOMIC_FETCH_ADD_UNLESS
#define HAVE_XARRAY
#else /* HAVE_MOFED */
#define MOFED_VERSION(a, b) (((a) << 8) + (b))
#if MOFED_VERSION(MOFED_MJVER, MOFED_MINVER) >= MOFED_VERSION(25, 7)
#define NEED_FUNC_IB_COPY_PATH_REC_FROM_USER
#define MODULE_IMPORT_NS_HAS_STRING
#endif
#endif /* HAVE_MOFED */

#include "compat_common.h"

#endif /* SLES15SP7_COMPAT_H */
