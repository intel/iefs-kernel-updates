...
#if defined(NVIDIA_GPU_DIRECT) || defined(INTEL_GPU_DIRECT)
#define RV_CAP_INTEL_GPU (1UL << 61) /* Intel GPU support */
#define RV_CAP_NVIDIA_GPU (1UL << 62) /* Nvidia GPU support */
#define RV_CAP_GPU_DIRECT (1UL << 63) /* GPU Direct Copy, DMA & RDMA support */
/* mask to indicate if any GPU features in capability */
#define RV_CAP_GPU_MASK (RV_CAP_INTEL_GPU|RV_CAP_NVIDIA_GPU|RV_CAP_GPU_DIRECT)
#endif

struct rv_capability_params {
		/* ABI version */
	__u16 major_rev;
	__u16 minor_rev;
#if defined(NVIDIA_GPU_DIRECT) || defined(INTEL_GPU_DIRECT)
...
/* older query ABI has same structure, but _IOR vs _IOWR */

#define RV_IOCTL_CAPABILITY _IOWR(RV_MAGIC, 0xFC, struct rv_capability_params)
#define RV_IOCTL_QUERY_R2   _IOR(RV_MAGIC, 0xFC, struct rv_capability_params)
...
#if defined(NVIDIA_GPU_DIRECT) || defined(INTEL_GPU_DIRECT)
#define RV_RDMA_MODE_GPU_ONLY 2 /* Only for GPU access (pin/mmap only) */
...
struct rv_attach_params_in {
	...
	__u32 hb_interval;
	__u32 fr_page_list_len;
};

/* RV_ABI_VER_MINOR 5 */
struct rv_attach_params_r5_in {
	char dev_name[RV_MAX_DEV_NAME_LEN];
	__u32 mr_cache_size;
	__u8 rdma_mode;
 ...
 * max_fmr_size - max fast-registration mr size in bytes.
 */
...
#if defined(NVIDIA_GPU_DIRECT) || defined(INTEL_GPU_DIRECT)
		/* Note: sizeof(out_gpu) <= sizeof(in) so no padding needed */
		struct rv_attach_params_out_gpu out_gpu;
#endif
	};
};

struct rv_attach_params_r5 {
	union {
		struct rv_attach_params_r5_in in;
		struct rv_attach_params_out out;
#if defined(NVIDIA_GPU_DIRECT) || defined(INTEL_GPU_DIRECT)
		/* Note: sizeof(out_gpu) <= sizeof(in) so no padding needed */
		struct rv_attach_params_out_gpu out_gpu;
...
#define RV_IOCTL_ATTACH_R5	_IOWR(RV_MAGIC, 0xF1, struct rv_attach_params_r5)
...
struct rv_mem_params_r1 {
	union {
		struct rv_mem_params_r1_in in;
		struct rv_mem_params_out out;
	};
};

struct rv_mem_params_r4 {
	union {
		struct rv_mem_params_r4_in in;
		struct rv_mem_params_out out;
	};
};

#define RV_IOCTL_REG_MEM	_IOWR(RV_MAGIC, 0xF3, struct rv_mem_params)
#define RV_IOCTL_REG_MEM_R1	_IOWR(RV_MAGIC, 0xF3, struct rv_mem_params_r1)
#define RV_IOCTL_REG_MEM_R4	_IOWR(RV_MAGIC, 0xF3, struct rv_mem_params_r4)
...
/* RV_GPU_ABI_VER_MINOR 0 */
struct rv_gpu_mem_params_r0 { /* maps to hfi1_gdr_query_params */
	union {
		struct rv_gpu_mem_params_r0_in {
			__aligned_u64 gpu_buf_size;
			__aligned_u64 gpu_buf_addr;
			__u32 access;
#ifdef INTEL_GPU_DIRECT
...
#define RV_IOCTL_GPU_PIN_MMAP_R0 _IOWR(RV_MAGIC, 0xF5, struct rv_gpu_mem_params_r0)
