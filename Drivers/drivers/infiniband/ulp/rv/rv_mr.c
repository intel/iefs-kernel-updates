...
#if defined(RV_REG_MR_DISCRETE) || defined(NVIDIA_GPU_DIRECT)
...
static struct mutex mr_pd_uobject_lock;
#endif
			  ...
			  bool signaled)
{
	...
	pool->cqe.done = rv_fr_inv_rkey_done;
	wr.wr_cqe = &pool->cqe;
...
#endif /*RV_REG_MR_DISCRETE || NVIDIA_GPU_DIRECT */
		       ...
		       struct mr_info *mr)
{
	...
	if (minfo->access & IBV_ACCESS_KERNEL) {
		...
		if (rv->rdma_mode != RV_RDMA_MODE_KERNEL || !rv->jdev) {
			rv_err(rv->inx, "kernel access in USER mode\n");
			ret = -EINVAL;
			goto err_put;
		}
		/*
...
static void rv_user_mrs_destroy_rc_qp(struct rv_user_mrs *umrs)
{
#ifdef NVIDIA_GPU_DIRECT
	if (umrs->user_qp) {
		trace_rv_msg_destroy_rc_qp(umrs->rv_inx, umrs->dev,
	...
	if (umrs->qp) {
		trace_rv_msg_destroy_rc_qp(umrs->rv_inx, umrs->jdev->dev,
			   ...
			   struct rv_device *dev, u8 port_num)
{
	...
	ret = rdma_query_gid(dev->ib_dev, port_num, 0, &gid);
	if (ret) {
	...
	if (!umrs->send_cq) {
		umrs->send_cq = ib_alloc_cq(dev->ib_dev, umrs, 10, 0,
					    IB_POLL_SOFTIRQ);
	...
	if (!umrs->recv_cq) {
		umrs->recv_cq = ib_alloc_cq(dev->ib_dev, umrs, 10, 0,
					    IB_POLL_SOFTIRQ);
	...
	*qp = ib_create_qp(pd, &init_attr);
	...
	trace_rv_msg_create_rc_qp(umrs->rv_inx, dev, "RC qp is in RTS",
				  (u64)(*qp)->qp_num, 0);
	return 0;
fail:
	if (*qp) {
...
static void rv_destroy_fr_pool(struct rv_fr_pool *pool)
{
	if (!pool)
		return;
	...
	/*
	 * We should not have any entry on the used_list. Nevertheless, try
	 * to free if any.
	 */
...
static int rv_fr_desc_batch_alloc(struct rv_fr_pool *pool)
{
	int i;
	...
	desc = kzalloc(sizeof(*desc) * fr_batch_size, GFP_KERNEL);
	if (!desc)
		return -ENOMEM;
	for (i = 0; i < fr_batch_size; i++) {
...
static struct rv_fr_pool *rv_create_fr_pool(struct rv_user_mrs *umrs)
{
	...
	if (!pool)
		goto err;
	pool->max_page_list_len = RV_MAX_PAGE_LIST_LEN;
	pool->umrs = umrs;
	...
	ret = rv_fr_desc_batch_alloc(pool);
	if (ret)
		goto destroy_pool;
out:
	return pool;
...
static void rv_fr_pool_put(struct rv_fr_desc *desc)
{
	...
	mutex_lock(&pool->lock);
	/*
	 * We don't need to invalidate the rkey when the umrs is being
	 * released.
	 */
	...
	if (cnt > 0 && !(cnt % RV_RC_QP_INV_THRESHOLD)) {
		...
		if (!ret) {
			...
			if (atomic_read(&pool->inv_cnt) >= RV_RC_QP_INV_LIMIT)
...
move:
	...
	list_move_tail(&desc->entry, &pool->free_list);
	pool->free_size++;
	mutex_unlock(&pool->lock);
}
